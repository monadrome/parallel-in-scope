...
`BatchOptions`、`TaskOptions` 等选项类型用静态工厂加不可变 wither；只有需要多步装配的 `GlobalPar` 与 `TaskGroupDefinition` 保留 Builder。虽然比 Record 多了不少代码，但带来了额外的好处：
...
> 相关文章：[parallel-in-scope 设计复盘](../../../../docs/zh/design/philosophy.md)
