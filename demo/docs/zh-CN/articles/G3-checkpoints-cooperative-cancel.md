...
for (int i = 0; i < 1_000_000; i++) {
    Checkpoints.checkpoint("cpu-task", true); // 任务名必须与 BatchOptions 一致
...
配合 `BatchOptions.timeout()` 设置任务超时，框架在超时后自动触发取消。对于 IO 任务（`Thread.sleep`、阻塞读写），中断机制天然有效；对于 CPU 任务，在循环体内插入 `Checkpoints.checkpoint(taskName, lean)` 即可及时响应取消。
