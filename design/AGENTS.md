...
| 文档 | 摘要 |
|---|---|
| [task-group-api-and-options.md](task-group-api-and-options.md) | TaskGroup 目标与非目标、Group/Batch 语义边界、`GlobalPar.defineGroup*`/`TaskGroupDefinition`/`Member`/`TaskGroup`/`Bindings` 公共 API、选项类型（`BatchOptions`/`TaskOptions` + `Builder.closeGrace`）、结果类型（`TaskGroupResult`/`TaskOutcome`） |
...
| [task-group-submission.md](task-group-submission.md) | `GlobalPar.submitGroup` 冻结与统一提交契约、配置期校验、executor rejection、两阶段提交内核 `TaskSubmissions` 的复用边界 |
