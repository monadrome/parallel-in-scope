# queue 包产物边界决策（决策分析，待拍板）

> 状态：**决策分析，待拍板**。本文自包含地说明 queue 包产物边界的全部相关事实、
> 选项与推荐，无需查阅其他文档。拍板后方向以 ADR 固化，本文随即归档。
...
**推荐选项 A（拆分），拍板后以 ADR 固化方向。** 理由：queue 包"独立通用
产品"的定位保留（推翻它需要先修订既定定位，无此诉求），物理拆分让 core
评审面与缺陷归属各归其位；0.x 窗口内做，用户经传递依赖无感。

**退路：若不愿为本库引入多模块构建，则选 C**——维持现状并以 ADR 记录
"这是有意承担的评审面"。这是诚实的终态，优于悬置。

## 7. 拍板问题清单

1. 是否接受为多模块构建/发布付出长期维护成本？——是则 A，否则 C。
2. 若选 A：queues 产物的 artifactId 与版本策略（与 core 同版本同发布，
   还是独立版本线）？
3. 若选 C：ADR 中评审面措辞的确认（21% 评审面 + 独立契约 + 零耦合，
   作为有意承担的结论写入）。
