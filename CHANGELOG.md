...
- Replace `ParName` with plain `String` at every naming endpoint: `ParRuntime.Builder.register(String, ExecutorService)` still returns `Builder`, and `defaultPar(String)`, `parTaskListener(String, TaskListener)`, `ParRuntime.par` / `find` / `taskListenersFor(String)` now take `String`; `ParRuntime.pars()` returns `Map<String, Par>` and `Par.name()` returns `String`. Null/blank validation moves into the endpoints (`NullPointerException` / `IllegalArgumentException` at the call site); `Builder.build()` keeps its registration-consistency checks.
